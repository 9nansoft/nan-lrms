...
export function buildVersionRejectionMessage(result: BmsVersionCheckResult): string {
  ...
  if (result.reason === 'too_old') {
    return `HOSxP API ของโรงพยาบาลเป็นเวอร์ชัน ${result.version ?? '-'} ซึ่งเก่ากว่าที่ระบบ NN-LRMS ต้องการ (${min} ขึ้นไป) — กรุณาอัปเดต HOSxP เป็นเวอร์ชันใหม่ก่อนเข้าใช้งาน`;
  }
