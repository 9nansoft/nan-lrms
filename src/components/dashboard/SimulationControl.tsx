  ...
  return (
    <>
      <button
        ...
        className={cn(
          'inline-flex shrink-0 items-center gap-1.5 rounded-sm px-2.5 py-1.5 font-mono text-[13px] font-semibold tracking-[0.06em] transition-colors',
          status.running
        ...
        >
          ...
          >
            <div>
              ...
              <DialogDescription className="font-mono text-[13px] tracking-[0.1em] text-white/85">
                Synthetic events generated by Gemma-4 · writes to local DB · DEV ONLY
              </DialogDescription>
            </div>
            {status.running && (
              <span className="rounded-sm bg-white px-2 py-1 font-mono text-[13px] font-semibold text-amber-700">
          ...
          <div className="grid h-full overflow-hidden" style={{ gridTemplateColumns: '1fr 360px' }}>
            ...
            <div className="overflow-auto px-5 py-4" style={{ maxHeight: '78vh' }}>
              ...
              <section>
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  {PRESETS.map((p) => (
                    ...
                    >
                      <div className="text-[15px] font-semibold text-[var(--ink-navy)]">{p.label}</div>
                      <div className="mt-0.5 text-[13px] text-[var(--ink-navy-muted)]">
                        {p.description}
                      </div>
                    </button>
                  ))}
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {ALL_EVENT_TYPES.map((et) => (
                    <label
                      key={et.key}
                      className={cn(
                        'inline-flex cursor-pointer items-center gap-1.5 rounded-sm border px-2.5 py-1 text-[14px]',
              ...
              {presetId === 'custom' || scenario ? (
                <section className="mt-5">
                  ...
                  <textarea
                    ...
                    className="mt-2 w-full rounded-sm border border-[var(--rule-strong)] bg-white px-3 py-2 text-[15px] focus:border-amber-500 focus:outline-none"
                  />
                </section>
              ) : null}
              ...
              <section className="mt-5">
                ...
                <select
                  ...
                  className="mt-2 w-full rounded-sm border border-[var(--rule-strong)] bg-white px-3 py-1.5 text-[15px] focus:border-amber-500 focus:outline-none"
                >
                  {models.map((m) => (
                ...
                {modelsErr && (
                  <div className="mt-1 flex items-center gap-1 font-mono text-[12px] text-amber-700">
                ...
                <div className="mt-2 grid grid-cols-2 gap-1.5">
                  {KK_HOSPITALS.map((h) => (
                    <label
                      key={h.hcode}
                      className={cn(
                        'flex cursor-pointer items-center gap-2 rounded-sm border px-2 py-1 text-[14px] transition-colors',
              ...
              <section className="mt-5 rounded-sm border border-red-200 bg-red-50/60 p-3">
                <h3 className="font-mono text-[12px] uppercase tracking-[0.14em] text-red-700">
                  08 · Data management
                </h3>
                <div className="mt-2 flex flex-wrap items-center justify-between gap-3">
                  <label
                    className={cn(
                      'inline-flex cursor-pointer items-center gap-2 rounded-sm border px-2.5 py-1.5 text-[14px]',
                      startFresh
                  ...
                  {!confirmClear ? (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-sm border border-red-400 bg-white px-3 py-1.5 font-mono text-[13px] font-semibold tracking-[0.06em] text-red-600 hover:bg-red-100 disabled:opacity-40"
                      title={
                        status.running
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <span className="font-mono text-[12px] text-[var(--ink-navy-dim)]">
                      ...
                      <div className="mt-1 flex items-center gap-2">
                        <button
                          ...
                          disabled={clearing}
                          className="rounded-sm border border-[var(--rule-strong)] bg-white px-2.5 py-1 font-mono text-[12px] tracking-[0.06em] text-[var(--ink-navy-dim)] hover:bg-slate-50"
                        >
                          Cancel
                        </button>
                        <button
                          ...
                          onClick={onClear}
                          disabled={clearing}
                          className="inline-flex items-center gap-1 rounded-sm bg-red-600 px-3 py-1 font-mono text-[12px] font-semibold tracking-[0.06em] text-white hover:bg-red-700 disabled:opacity-60"
                        >
                ...
                {clearResult && (
                  <div className="mt-2 font-mono text-[12px] text-[var(--ink-navy-dim)]">
                ...
                <div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-red-200 pt-3">
                  <div className="text-[13px] leading-snug text-[var(--ink-navy-dim)]">
                  ...
                  {!confirmResetOnboarding ? (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-sm border border-red-500 bg-white px-3 py-1.5 font-mono text-[13px] font-semibold tracking-[0.06em] text-red-700 hover:bg-red-100 disabled:opacity-40"
                      title={
                        status.running
                          ? 'Stop the running simulation first'
                    ...
                    <div className="flex flex-col gap-1">
                      <span className="font-mono text-[12px] font-semibold text-red-800">
                      ...
                      <div className="mt-1 flex items-center gap-2">
                        ...
                        <button
                          ...
                          disabled={resettingOnboarding}
                          className="inline-flex items-center gap-1 rounded-sm bg-red-700 px-3 py-1 font-mono text-[12px] font-semibold tracking-[0.06em] text-white hover:bg-red-800 disabled:opacity-60"
                        >
                          <Trash2 className="h-3 w-3" />
              ...
              <section className="mt-6 flex items-center justify-between gap-3 border-t pt-4"
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="rounded-sm border border-[var(--rule-strong)] bg-white px-4 py-1.5 font-mono text-[13px] tracking-[0.06em] text-[var(--ink-navy-dim)] hover:bg-slate-50"
                  >
                    Cancel
                  </button>
                  {status.running ? (
                    <button
                      onClick={onStop}
                      className="inline-flex items-center gap-1.5 rounded-sm bg-red-500 px-4 py-1.5 font-mono text-[13px] font-semibold tracking-[0.06em] text-white hover:bg-red-600"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-sm bg-amber-500 px-4 py-1.5 font-mono text-[13px] font-semibold tracking-[0.06em] text-white hover:bg-amber-600 disabled:opacity-50"
                    >
            ...
            >
              <div className="border-b p-4 font-mono text-[12px] uppercase tracking-[0.14em] text-[var(--ink-navy-muted)]"
                   style={{ borderColor: 'var(--rule-strong)' }}>
                Live status
              </div>
              {!status.running && status.hospitals.length === 0 ? (
                <div className="p-6 text-center font-mono text-[13px] text-[var(--ink-navy-muted)]">
                ...
                <div className="flex h-full flex-col overflow-hidden">
                    ...
                    {status.hospitals
                      ...
                      .map((h) => (
                        ...
                        >
                          ...
                          {h.lastError && (
                            <div className="truncate font-mono text-[11px] text-red-500" title={h.lastError}>
                              {h.lastError}
                            </div>
                          )}
                        </div>
                  ...
                  <div className="flex-shrink-0 border-t p-3"
                    ...
                    <div className="mt-2 max-h-40 overflow-auto font-mono text-[12px]">
